use std::sync::Arc;
use poem::{get, handler, http::StatusCode, listener::TcpListener, post, web::{Data, Json, Query, Path}, EndpointExt as _, IntoResponse, Response, Route, Server};
...
async fn get_remote_servers(
    ...
    let links = (0..last_page).map(|i| crate::datamodel::get::api::remote::servers::response::meta::Link {
        url: Some(format!("{}/api/remote/servers?page={}&per_page={}", state.path, i, per_page)),
        label: i.to_string(),
        active: i == page,
    }).collect::<Vec<crate::datamodel::get::api::remote::servers::response::meta::Link>>();

    let links = vec![crate::datamodel::get::api::remote::servers::response::meta::Link {
        url: None,
        label: "&laquo; Previous".to_owned(),
        active: false
    }].into_iter().chain(links).chain(vec![crate::datamodel::get::api::remote::servers::response::meta::Link {
        url: None,
        label: "Next &raquo;".to_owned(),
        active: false
    }].into_iter()).collect::<Vec<crate::datamodel::get::api::remote::servers::response::meta::Link>>();

    let res = crate::datamodel::get::api::remote::servers::Response {
        data: state.servers.clone().windows(per_page as usize).nth(page as usize - 1).unwrap_or_default().to_vec(),
        ...
        links: crate::datamodel::get::api::remote::servers::response::Links {
            prev: if page > 1 {
                Some(format!("{}/api/remote/servers?page={}&per_page={}", state.path, page - 1, per_page))
            } else {
                None
            },
            ...
            first: format!("{}/api/remote/servers?page=1&per_page={}", state.path, per_page),
            last: format!("{}/api/remote/servers?page={}&per_page={}", state.path, last_page, per_page),
        }
    };
...
async fn add_remote_server_activity(
    ...
    for server in body.data.iter() {
        let server = state.servers.iter().find(|s| s.uuid == uuid::Uuid::parse_str(&server.server).unwrap_or_default());
        if server.is_none() {
...
async fn main() -> Result<(), std::io::Error> {
    ...
    let app = Route::new()
        ...
        .at("/api/remote/servers/:uuid/install", get(get_remote_server_installations).post(update_remote_server_installation))
        .data(state);
